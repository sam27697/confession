...
Usage (run from the repository root):
  python3 scripts/authorship_scan.py --base REV [--head REV]
          [--allow-file scripts/authorship_allow.txt]

Came from the UX program's kit (.uxprogram/kit/tools/, retired 2026-09-23) and
is kept because the rule it enforces outlived the program: no authorship tells
in this repository's history. Run it against the commit your work started from
before you push.
...
# This file and its allowlist necessarily spell out every pattern they ban,
# and .uxprogram/ was the retired program's own bookkeeping; none of them is
# product.
SKIP_PREFIXES = (".uxprogram/", "scripts/authorship_scan.py", "scripts/authorship_allow.txt")
...
def main():
    ...
    p.add_argument("--allow-file", default="scripts/authorship_allow.txt")
    a = p.parse_args()
