...
import { AFTER_LOGIN_COOKIE, FB_OAUTH_STATE_COOKIE } from '../../../_lib/session.js'
...
export async function GET(request: Request) {
  ...
  // Set by /auth/facebook/start from its ?next= (week 15 §2.3).
  ...
  await resolveLoginAndRedirect(
    { provider: 'facebook', providerUserId: profile.id, displayName: profile.name },
    remembered,
  )
