...
// `accounts` and `links` were imported here too and are not used: every
// assertion about those two tables in this file goes through raw SQL against
// the pool, on purpose, so it reads what Postgres holds rather than what the
// ORM believes. Dropping the two unused names is what makes `tsc --noEmit`
// pass; no assertion in this file changes. (week 10 review, 2026-09-02)
import { confessions, adminUsers } from '../src/schema.js'
...
//
// AMENDED 2026-09-02, by the reviewer, not by the implementer. See spec
// section 6.1. The English discriminator was 'delete your account at any
// time', which is also a substring of FROZEN_CLAUSE_6_EN above, so items 26
// and 27 were mutually unsatisfiable: item 26 demands the English clause 6 be
// byte-identical to that literal, item 27 forbade any clause containing part
// of it. No value of TERMS_TEXT_EN.clauses[5] could pass both, whatever the
// implementation did. The Arabic constant never had this problem because it
// carries the old clause's «أو» ("or"), which the rewritten clause replaces
// with «وفيك». The English constant now carries the same discriminator, the
// old clause's "off or", so it still catches the real pre-week-10 wording in
// any clause that embeds it. Proven by mutation, not by argument: the
// pre-week-10 clause 6 was pasted back into src/terms.ts and item 27 went red.
